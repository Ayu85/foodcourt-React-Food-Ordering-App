...
const RestaurantCard = ({ name, cloudinaryImageId, locality, areaName, costForTwo, avgRating }) => {
    return (
        <div className='flex flex-col py-2 px-2 gap-2 items-center card-font   shadow-[#facdcd8f] shadow-xl  bg-white justify-center '>
            ...
            <div className=' text-black  bottom-0 text-center   w-full rounded-3xl  '> <h1 className='text-lg font-semibold   bottom-5'>{name}</h1>
                <h1 className=' text-md text-slate-700'>{locality},{areaName}   </h1></div>
        </div>
    )
}
